...
namespace AdimeoDataSuite\SearchClientBundle\Classes;
...
class SearchContext
{
  const ADS_STATUS_IDLE = 1;
  const ADS_STATUS_EXECUTED = 2;
   ...
   * @var int
   */
  private $status = SearchContext::ADS_STATUS_IDLE;
  ...
  public function execute($params = null){
    ...
    $this->status = SearchContext::ADS_STATUS_EXECUTED;
  ...
  private function getResponse($url){
    ...
    if($response->getStatusCode() == 200) {
      ...
      throw new \Exception("CtSearch response failed => code " . $response->getStatusCode() . ". Response is " . $response->getBody());
    }
  }
